...
<div class="btn-toolbar list-toolbar">
    ...
    <form class="form-inline">
        ...
        <% } else { %>
            ...
            <h3>Вы искали: <b>${phraseForSearch}</b></h3>
            <p class="stat"><span class="label label-success"><%=usersService.currentEntityTable.searchOutRowsNumbers.size()%></span> Detected records (обнаруженные записи) </p>
            <p class="stat"><span class="label label-info"><%=usersService.currentEntityTable.searchInstancesNumber%></span> Detected instances (количество вхождений) </p>
            <h2> </h2>
...
<table class="table">
  ...
  <%  List detectedRecords = usersService.currentEntityTable.searchOutRowsNumbers;
      for(int i = 0; i < detectedRecords.size(); i++) {
          List<TableField> tmpList = usersService.currentEntityTable.rows.get( detectedRecords.get(i) );
          List detectedFields = usersService.currentEntityTable.searchOutTableFieldsNumbers.get( detectedRecords.get(i) );
  %>
      <tr>
          <% for(int k = 0; k < tmpList.size(); k++) {
                if ( detectedFields.contains(k) ) { %>
                    <td><b><font color="#5ba9de"><%= tmpList.get(k).value %><font/></b></td>
                <% } else { %>
      ...
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-footer">
                  ...
                  <a class="btn btn-danger" href="/delete?RecordId=<%= tmpList.get(0).value %>" data-dismiss="modal">Delete</a>
              </div>
          </div>
      </div>
  </div>
